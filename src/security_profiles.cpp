	...
	VeQItemAction()
{
	...
	mPendingServiceRestart = pltService->itemGetOrCreateAndProduce("Network/PendingServiceRestart", 0);
};
...
int SecurityApi::setValue(const QVariant &value)
{
	...
	if (password.isValid()) {
		...
		reauthenticated = true; // Trigger user to login again
	}
	...
	if (reauthenticated)
		mPendingServiceRestart->setValue(mPendingServiceRestart->getLocalValue().toInt() + 1);
...
void SecurityProfiles::onMqttAccessChanged(QVariant const &var)
{
	if (mMqttAccess != var) {
		...
		//
		// CHECK THIS: should this not report websocket support as well nowadays??
		if (wasValid) {
...
void SecurityProfiles::enableMqtt(bool enabled, bool restart)
{
...
void SecurityProfiles::checkMqtt(bool restart)
{
