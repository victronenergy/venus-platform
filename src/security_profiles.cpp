...
QString passwordFileName = QStringLiteral("/data/conf/vncpassword.txt");
...
int SecurityApi::setValue(const QVariant &value)
{
	...
	if (securityProfile.isValid()) {
		...
		case SECURITY_PROFILE_INDETERMINED:
...
void SecurityProfiles::checkPassword()
{
	...
	if (mSecurityProfile.toInt() == SECURITY_PROFILE_INDETERMINED) {
		if (hasPasswordFile())
