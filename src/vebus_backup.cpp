	...
	QObject(parent)
{
	...
	mMk2DbusFirmwareVersionItem->getValueAndChanges(this, SLOT(onVebusFirmwareVersionChanged(QVariant)));

	mMk2DbusSubVersionItem = service->item("FirmwareSubVersion");
	mMk2DbusSubVersionItem->getValueAndChanges(this, SLOT(onVebusFirmwareSubVersionChanged(QVariant)));

	mMk2DbusProductIdItem = service->item("ProductId");
	mMk2DbusProductIdItem->getValueAndChanges(this, SLOT(onVebusProductIdChanged(QVariant)));
...
void VebusBackupService::getAvailableBackups()
{
	...
	QVariant connected = mMk2DbusConnectedItem->getValue();
	foreach(QString fileName, backupFiles) {
		...
		if (proc.exitStatus() == QProcess::NormalExit) {
			if (proc.exitCode() == 0) {
				...
				if (connected.isValid() && (connected.toInt() != 0)) {
					...
					// VE.Bus firmware versions and product ids are unknown
					// Add all files to AvailableBackupFiles
					if (i > 0) {
						backupFilesString.append(", ");
					}
					backupFilesString.append("\"" + fileName + "\"");
...
bool VebusBackupService::checkFirmwareVersionCompatibility(const QString& prodIdAndVersion, const QString& subVersion)
{
	...
	if (vebusFirmwareVersionNumber.isEmpty() || vebusFirmwareSubVersionNumber.isEmpty() || vebusProductId.isEmpty()) {
		return false;
	}
...
void VebusBackupService::onVebusFirmwareVersionChanged(QVariant var)
{
	if (var.isValid()) {
		vebusFirmwareVersionNumber = QString::number(var.toUInt(), 16);
	}
}

void VebusBackupService::onVebusFirmwareSubVersionChanged(QVariant var)
{
	if (var.isValid()) {
		vebusFirmwareSubVersionNumber = QString::number(var.toUInt(), 16);
	}
}

void VebusBackupService::onVebusProductIdChanged(QVariant var)
{
	if (var.isValid()) {
		vebusProductId = QString::number(var.toUInt(), 16);
	}
}
