...
DBusService::DBusService(VeQItem *serviceItem, DBusServiceType::ServiceType serviceType, QObject *parent) :
	QObject(parent),
...
DBusService *DBusService::createInstance(VeQItem *serviceItem)
{
	...
	if (type == "battery")
		return new DBusService(serviceItem, ServiceType::DBUS_SERVICE_BATTERY);

	if (type == "vebus")
		return new DBusService(serviceItem, ServiceType::DBUS_SERVICE_MULTI);

	if (type == "multi")
		return new DBusService(serviceItem, ServiceType::DBUS_SERVICE_MULTI_RS);

	if (type == "solarcharger")
		return new DBusService(serviceItem, ServiceType::DBUS_SERVICE_SOLAR_CHARGER);

	if (type == "pvinverter")
		return new DBusService(serviceItem, ServiceType::DBUS_SERVICE_PV_INVERTER);

	if (type == "charger")
		return new DBusService(serviceItem, ServiceType::DBUS_SERVICE_AC_CHARGER);

	if (type == "tank")
		return new DBusTankService(serviceItem);

	if (type == "grid")
		return new DBusService(serviceItem, ServiceType::DBUS_SERVICE_GRIDMETER);

	if (type == "genset")
		return new DBusService(serviceItem, ServiceType::DBUS_SERVICE_GENSET);

	if (type == "motordrive")
		return new DBusService(serviceItem, ServiceType::DBUS_SERVICE_MOTOR_DRIVE);

	if (type == "inverter")
		return new DBusService(serviceItem, ServiceType::DBUS_SERVICE_INVERTER);

	if (type == "system")
		return new DBusService(serviceItem, ServiceType::DBUS_SERVICE_SYSTEM_CALC);

	if (type == "temperature")
		return new DBusService(serviceItem, ServiceType::DBUS_SERVICE_TEMPERATURE_SENSOR);

	if (type == "generator")
		return new DBusService(serviceItem, ServiceType::DBUS_SERVICE_GENERATOR_STARTSTOP);

	if (type == "pulsemeter")
		return new DBusService(serviceItem, ServiceType::DBUS_SERVICE_PULSE_COUNTER);

	if (type == "digitalinput")
		return new DBusService(serviceItem, ServiceType::DBUS_SERVICE_DIGITAL_INPUT);

	if (type == "unsupported")
		return new DBusService(serviceItem, ServiceType::DBUS_SERVICE_UNSUPPORTED);

	if (type == "meteo")
		return new DBusService(serviceItem, ServiceType::DBUS_SERVICE_METEO);

	if (type == "vecan")
		return new DBusService(serviceItem, ServiceType::DBUS_SERVICE_VECAN);

	if (type == "evcharger")
		return new DBusService(serviceItem, ServiceType::DBUS_SERVICE_EVCHARGER);

	if (type == "acload")
		return new DBusService(serviceItem, ServiceType::DBUS_SERVICE_ACLOAD);

	if (type == "hub4")
		return new DBusService(serviceItem, ServiceType::DBUS_SERVICE_HUB4);

	if (type == "fuelcell")
		return new DBusService(serviceItem, ServiceType::DBUS_SERVICE_FUELCELL);

	if (type == "dcsource")
		return new DBusService(serviceItem, ServiceType::DBUS_SERVICE_DCSOURCE);

	if (type == "alternator")
		return new DBusService(serviceItem, ServiceType::DBUS_SERVICE_ALTERNATOR);

	if (type == "dcload")
		return new DBusService(serviceItem, ServiceType::DBUS_SERVICE_DCLOAD);

	if (type == "dcsystem")
		return new DBusService(serviceItem, ServiceType::DBUS_SERVICE_DCSYSTEM);

	if (type == "platform")
		return new DBusService(serviceItem, ServiceType::DBUS_SERVICE_PLATFORM);
...
std::vector<QString> const &DBusTankService::knownFluidTypes()
{
	...
	if (fluidTypes.size() == 0) {
		...
		fluidTypes.push_back(tr("Gasoline"));
		fluidTypes.push_back(tr("Diesel"));
		fluidTypes.push_back(tr("LPG"));
		fluidTypes.push_back(tr("LNG"));
		fluidTypes.push_back(tr("Hydraulic oil"));
		fluidTypes.push_back(tr("Raw water"));
	}
...
DBusTankService::DBusTankService(VeQItem *serviceItem, QObject *parent) :
	DBusService(serviceItem, ServiceType::DBUS_SERVICE_TANK, parent)
{
...
void DBusTankService::updateDescription()
{
	VeQItem *customNameItem = item("CustomName");
	QString customName = customNameItem->getValue().toString();
